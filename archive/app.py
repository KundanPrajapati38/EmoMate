# app.py (Final Version - Gemini Edition)
...
# --- TensorFlow optional load (not supported on Python 3.14) ---
try:
    from tensorflow.keras.models import load_model as tf_load_model
    TF_AVAILABLE = True
except ImportError:
    TF_AVAILABLE = False
    print("TensorFlow not available on this Python version. Image emotion detection will be disabled.")
...
try:
    ...
    print("Zhipu AI (GLM) client initialized successfully!")
except Exception as e:
    print(f"Error initializing Zhipu AI client: {e}")
    zhipu_client = None
...
try:
    if TF_AVAILABLE:
        ...
        EMOTION_MODEL = tf_load_model('model.h5')
        EMOTION_LABELS = ['Angry', 'Disgust', 'Fear', 'Happy', 'Neutral', 'Sad', 'Surprise']
        print("Image emotion models loaded successfully!")
    else:
        raise ImportError("TensorFlow not available")
except Exception as e:
    print(f"Image emotion models not loaded: {e}")
    FACE_CASCADE = None
    EMOTION_MODEL = None
...
def chat():
    if not zhipu_client:
        return jsonify({"error": "Zhipu AI client not initialized."}), 500
    try:
        user_message = request.json['message']
        if not user_message:
            return jsonify({"error": "Empty message"}), 400

        chat_completion = zhipu_client.chat.completions.create(
            model="glm-4-flash",
            messages=[
                {
                    "role": "system",
                    "content": "You are an empathetic AI companion called EmoMate. Your goal is to help the user with their emotional wellbeing. Detect their primary emotion and provide a short, supportive, conversational response (max 30 words). Your final output must be a JSON object with two keys: \"emotion\" and \"reply\"."
                },
